...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BINGs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBingPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBingPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Minimum emission rate: 0.1 BING per block.
    ...
    // The block number when BING mining starts.
    uint256 public startBlock;

    // Bing referral contract address.
    IBingReferral public bingReferral;
    ...
    constructor(
        ...
        bingPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's BING allocation point and deposit fee. Can only be called by the owner.
    ...
    // Deposit LP tokens to MasterChef for BING allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeBingTransfer(msg.sender, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accBingPerShare).div(1e12);
    ...
    function safeBingTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeBingTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate() public {
        ...
        uint256 newEmissionRate = bingPerBlock;
        ...
        if (newEmissionRate >= bingPerBlock) {
            return;
        }
    ...
    // Update the bing referral contract address by the owner
    function setBingReferral(IBingReferral _bingReferral) public onlyOwner {

        bingReferral = _bingReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(bingReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = bingReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                bing.mint(referrer, commissionAmount);
